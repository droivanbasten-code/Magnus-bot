/**
 * Magnus Bot — /groupimg — تغيير وقفل صورة الغروب
...
const CACHE = path.join(os.tmpdir(), "magnus_groupimg");
...
module.exports = {
  config: {
    name: "groupimg", aliases: ["gcimg", "صورة", "img"], version: "3.0", author: "DJAMEL",
  ...
  onStart: async function({ api, event, args, message }) {
    ...
    if (sub === "status") {
      ...
      return message.reply(locked ? "🔒 صورة الغروب مقفلة.\n↩️ /groupimg off لفك القفل." : "🔓 غير مقفلة.");
    }
    ...
    if (!imageUrl) return message.reply("📸 أرسل صورة مع الأمر /groupimg أو أرفق رابط.\n/groupimg off — فك القفل\n/groupimg status — الحالة");
